...
export default function Page() {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (session?.user) {
        ...
        if (profileData) {
          ...
          if (pendingRef) {
            prdService.applyReferralCode(pendingRef)
              .then(() => {
                sessionStorage.removeItem('pending_referral');
                return prdService.getProfile();
              })
              .then((refreshedProfile: any) => {
                if (refreshedProfile) {
                  setUser(prev => prev ? { ...prev, remainingDownloads: refreshedProfile.remaining_downloads } : prev);
                }
              })
              .catch(err => console.error('Referral apply error:', err));
          }
        } else {
  ...
  useEffect(() => {
    ...
    // Store referral code from URL for later use
